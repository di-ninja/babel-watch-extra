...
async function run({
  ...
  async function compileFile(filename){
    ...
    const sourceMappingURL = basename+'.map'
    const distFilepathMap = distFilepath+'.map'
    const relativeDir = path.relative(path.dirname(distFilepath), path.dirname(srcFilepath))
    const options = {
      sourceMaps,
      sourceFileName: path.join(relativeDir, basename),
    }
    let err
    try{
      const { code, map, ast } = await transformFileAsync(srcFilepath, {
        ...options,
        ast: true
      })
      traverse(ast, {
      ...
      map.file = basename
      if(sourceMaps){
        // await fs.writeFile(distFilepathMap, JSON.stringify(map))
        fs.writeFileSync(distFilepathMap, JSON.stringify(map))
      }
      // await fs.writeFile(distFilepath, codeOut)
      fs.writeFileSync(distFilepath, codeOut)
    ...
    if(err){
      ...
      try{
        ...
        const {code, map} = await transformAsync(errorSource, {
          ...options,
