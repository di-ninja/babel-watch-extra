...
async function run({
  ...
  async function compileFile(filename){
    ...
    try{
          ...
          const handleComments = async (comment)=>{
            ...
            for(const dep of dependencies){
              ...
              if(!compileDependenciesMapReverse[d]){
                compileDependenciesMapReverse[d] = new Set()
              }
              compileDependenciesMapReverse[d].add(filepath)
            }
      ...
      await fs.writeFile(distFilepath, code)
      await fs.writeFile(distFilepath+'.map', map)
      if(verbose){
  ...
  function compileThisFile(filename){
    return compileExtensions.some(ext=> ext === filename.slice(-1*compileExtensions.length))
  }
  
  async function compileDependentFiles(relativeFilename){
    ...
    for(const [depencency, dependentFiles] of Object.entries(compileDependenciesMapReverse)){
      ...
      else if(depencency.slice(-1)==='/'){
        if(relativeFilename.slice(0, depencency.length)===depencency){
          for(const f of dependentFiles){
            const file = path.join(srcPath, f)
            if(await fs.pathExists(file)){
              await compileFile(file)
  ...
  async function handleFile(event, filename, firstRun){
    ...
    if(!firstRun){
      await compileDependentFiles(relativeFilename)
    }
